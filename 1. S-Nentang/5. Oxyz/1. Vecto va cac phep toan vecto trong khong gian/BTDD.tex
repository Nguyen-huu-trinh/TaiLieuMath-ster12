...
% --- CÂU 10 ---
\questionLinesManual{0}{10}{Cho tứ diện \(ABCD\) và điểm \(G\) thỏa \(\vec{GA} + \vec{GB} + \vec{GC} = \vec{0}\) (\(G\) là trọng tâm của tứ diện). Gọi \(O\) là giao điểm của \(GA\) và mặt phẳng \((BCD)\). Trong các khẳng định sau, khẳng định nào sai?}{%
    \begin{tasks}(4)
        \task \(\vec{GA} = -2\vec{OG}\)
        \task \(\vec{GA} = 4\vec{OG}\)
        \task \(\vec{GA} = 3\vec{OG}\)
