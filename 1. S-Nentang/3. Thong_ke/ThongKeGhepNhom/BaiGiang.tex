...
\begin{center}
    {\Large\bfseries BẢNG TỔNG HỢP CÔNG THỨC THỐNG KÊ THPT}\\[4pt]
\end{center}

\vspace{0.3cm}

\textbf{1. Ký Hiệu Dùng Trong Mẫu Ghép Nhóm}
Cho mẫu số liệu ghép nhóm gồm $k$ nhóm $[a_i; a_{i+1})$ với tần số tương ứng là $n_i$:
\begin{itemize}
    \item \textbf{Kích thước mẫu (tổng số số liệu):} $n = \sum_{i=1}^{k} n_i = n_1 + n_2 + \dots + n_k$
    \item \textbf{Giá trị đại diện của nhóm $[a_i; a_{i+1})$:} $x_i = \dfrac{a_i + a_{i+1}}{2}$
    \item \textbf{Độ dài của nhóm $[a_i; a_{i+1})$:} $h_i = a_{i+1} - a_i$
\end{itemize}


\section*{2. Các Đại Lượng Đo Xu Hướng Trung Tâm (Toán 11)}

\begin{enumerate}
    \item \textbf{Số trung bình ($\bar{x}$):}
    \[
    \bar{x} = \frac{1}{n} \sum_{i=1}^{k} n_i x_i = \frac{n_1 x_1 + n_2 x_2 + \dots + n_k x_k}{n}
    \]

    \item \textbf{Trung vị ($M_e$):}
    Giả sử nhóm $p = [a_p; a_{p+1})$ chứa trung vị (nhóm đầu tiên có tần số tích lũy $n_1 + \dots + n_p \ge \frac{n}{2}$):
    \[
    M_e = a_p + \frac{\frac{n}{2} - (n_1 + n_2 + \dots + n_{p-1})}{n_p} \cdot (a_{p+1} - a_p)
    \]

\item \textbf{Tứ phân vị ($Q_1, Q_2, Q_3$):}
    \begin{itemize}
        \item \textbf{Bước 1:} Tìm nhóm $p = [a_p; a_{p+1})$ chứa tứ phân vị $Q_k$ ($k \in \{1, 2, 3\}$) bằng \(\dfrac{k.n}{4}.\)
        \item \textbf{Bước 2:} Áp dụng công thức tổng quát:
        \[
        Q_k = a_p + \frac{\frac{k \cdot n}{4} - (n_1 + \dots + n_{p-1})}{n_p} \cdot (a_{p+1} - a_p)
        \]
    \end{itemize}

    \item \textbf{Mốt ($M_o$):}
    Giả sử nhóm $j = [a_j; a_{j+1})$ có tần số lớn nhất $n_j$:
    \[
    M_o = a_j + \frac{n_j - n_{j-1}}{(n_j - n_{j-1}) + (n_j - n_{j+1})} \cdot (a_{j+1} - a_j)
    \]
\end{enumerate}

\section*{3. Các Đại Lượng Đo Độ Tán Sắc (Toán 12)}

\begin{enumerate}
    \item \textbf{Khoảng biến thiên ($R$):} 
    \[
    R = a_{k+1} - a_1
    \]

    \item \textbf{Khoảng tứ phân vị ($\Delta_Q$):} 
    \[
    \Delta_Q = Q_3 - Q_1
    \]

    \item \textbf{Phương sai ($s^2$):}
    \[
    s^2 = \frac{1}{n} \sum_{i=1}^{k} n_i (x_i - \bar{x})^2 = \left( \frac{1}{n} \sum_{i=1}^{k} n_i x_i^2 \right) - \bar{x}^2
    \]

    \item \textbf{Độ lệch chuẩn ($s$):}
    \[
    s = \sqrt{s^2}
    \]
\end{enumerate}
\section*{Công thức xác định Vùng giá trị ngoại lệ (Outliers)}

Khoảng tứ phân vị:
\[ IQR = Q_3 - Q_1 \]

Vùng giá trị hợp lệ (không chứa giá trị ngoại lệ):
\[ \left[ Q_1 - 1.5 \times IQR, \quad Q_3 + 1.5 \times IQR \right] \]

Giá trị $x$ là \textbf{giá trị ngoại lệ} nếu:
...
% --- CÂU 1 ---
\questionLinesManual{3}{1}{Khảo sát thời gian tự học trung bình vào buổi tối của học sinh lớp 12A, thu được mẫu số liệu ghép nhóm sau:
...
% --- CÂU 41 ---
\questionShortAnswer{3}{2}{Một công ty xây dựng khảo sát khách hàng xem họ có nhu cầu mua nhà ở mức giá (triệu đồng/m\(^2\)) nào. Kết quả khảo sát được ghi lại ở bảng sau:
...
\end{center}
% --- CÂU 2 ---
\questionLinesManual{0}{3}{Khoảng biến thiên của mẫu số liệu trên là}{%
    \begin{tasks}(4)
        \task 25
        \task 20
        \task 30
...
% --- CÂU 3 ---
\questionLinesManual{3}{4}{Mốt của mẫu số liệu ghép nhóm trên gần bằng}{%
    \begin{tasks}(4)
        \task 163,33.
        \task 162,67.
        \task 163,67.
...
% --- CÂU 4 ---
\questionLinesManual{25}{5}{Khoảng tứ phân vị của mẫu số liệu ghép nhóm trên gần bằng}{%
    \begin{tasks}(4)
        \task 9,34
        \task 9,42
        \task 9,35
...
% --- CÂU 5 ---
\questionLinesManual{18}{6}{Mẫu số liệu ghép nhóm thống kê về mức lương của các nhân viên ở công ty X (đơn vị: triệu đồng) được cho trong biểu đồ cột dưới đây:
...
% --- CÂU 14 ---
\questionShortAnswer{15}{7}{Một trại hè về Toán học tổ chức cuộc thi "Khám phá vũ trụ Toán học" cho các học sinh tham gia. Kết quả về điểm của các học sinh được cho trong bảng số liệu ghép nhóm sau:
...
% --- CÂU 15 ---
\questionShortAnswer{15}{8}{Vào mùa thu hoạch cam, bác nông dân đã thu gom và tiến hành thống kê khối lượng của các quả cam.  
...
% --- CÂU 1 ---
\questionLinesManual{8}{9}{Độ tuổi của các khách hàng nữ mua hàng thời trang trong một ngày của một cửa hàng được thống kê trong bảng tần số ghép nhóm như sau:
...
% --- CÂU 2 ---
\questionLinesManual{8}{10}{Trong một buổi tập luyện, một vận động viên điền kinh thực hiện 50 lần ném tạ và ghi lại khoảng cách ném được (tính bằng mét). Để phân tích khả năng của vận động viên, huấn luyện viên đã thống kê các cự li ném theo bảng sau:
...
% --- CÂU 3 ---
\questionLinesManual{12}{11}{Một nông trường trồng rừng tiến hành đo chiều cao (đơn vị: m) của 100 cây keo 3 năm tuổi để đánh giá sự phát triển của cây. Các số liệu thu thập được sẽ được sử dụng để phân tích và đưa ra nhận định về sự đồng đều trong sinh trưởng của rừng keo. Kết quả thu được cho ở biểu đồ sau:
...
% % --- CÂU 11 ---
% \questionShortAnswer{8}{12}{Bộ phận kiểm tra chất lượng sản phẩm dùng máy để đo (chính xác đến 0,001mm) độ dày của một chi tiết máy. Kết quả đo 50 sản phẩm được thống kê ở bảng sau:
...
% --- CÂU 14 ---
\questionShortAnswer{15}{12}{Hai trang trại I và II thử nghiệm nuôi một giống gà mới. Sau 6 tháng, người ta thu hoạch và thống kê cân nặng (đơn vị: kg) của các con gà đó và cho kết quả dưới dạng biểu đồ sau:
