...
% ====== CÂU 120 (Đúng/Sai) ======
\questionTrueFalse{8}{120}{Cho hàm số \( y = f(x) \) có đồ thị hàm số \( y = f'(x) \) như hình vẽ.
\begin{flushleft}
\begin{tikzpicture}[>=latex, scale=0.4]
    % Hệ trục tọa độ
    \draw[->] (-3.5,0) -- (2.5,0) node[below] {\tiny$x$};
    \draw[->] (0,-3) -- (0,3.5) node[right] {\tiny$y$};
    \node[below right] at (0,0) {$O$};
    
    % Đánh dấu các điểm
    \node[below left] at (-2,0) {\tiny$a$};
    \node[below left] at (-0.3,0) {\tiny$b$};
    \node[below left] at (1.5,0) {\tiny$c$};
