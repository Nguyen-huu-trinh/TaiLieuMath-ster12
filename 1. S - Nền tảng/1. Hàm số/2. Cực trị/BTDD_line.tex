...
% ====== CÂU 3 ======
\questionLinesManual{5}{2}{Cho hàm số \( y = f(x) \) có bảng biến thiên sau:
\begin{center}
\begin{tikzpicture}[>=stealth]
...
\questionLinesManual{5}{3}{Cho hàm số \( f(x) \) có bảng biến thiên như sau:
\begin{center}
\begin{tikzpicture}[>=stealth]
    \tkzTabInit[nocadre=false,lgt=1.2,espcl=2.5]
    {$x$ / 1, $f'(x)$ / 1}
...
\questionLinesManual{5}{4}{Cho hàm số \( y = f(x) \) xác định trên \(\mathbb{R} \setminus \{1\}\) có bảng xét dấu như sau:
\begin{center}
\begin{tikzpicture}[>=stealth]
    \tkzTabInit[nocadre=false,lgt=1.2,espcl=2]
    {$x$ / 1, $y'$ / 1}
...
\questionLinesManual{5}{5}{Cho hàm số \( y = ax^3 + bx^2 + cx + d \) (\( a, b, c, d \in \mathbb{R} \)) có đồ thị như hình vẽ bên. Số điểm cực trị của hàm số này là
\begin{center}
...
\questionLinesManual{5}{6}{Cho hàm số \( y = f(x) \) có đồ thị như hình vẽ
\begin{center}
\begin{tikzpicture}[>=latex, scale=0.8]
    % Hệ trục tọa độ
    \draw[->] (-3.5,0) -- (3.5,0) node[below] {$x$};
...
\questionLinesManual{5}{7}{Cho hàm số \( y = x^3 - 3x^2 + 2 \). Mệnh đề nào dưới đây đúng?}{%
\begin{tasks}(2)
...
\questionLinesManual{5}{8}{Hàm số \( y = \dfrac{2x + 3}{x + 1} \) có bao nhiêu điểm cực trị?}{%
\begin{tasks}(4)
\task 1
\task 3
\task 0
...
\questionLinesManual{5}{9}{Cho hàm số \( f(x) \) có đạo hàm \( f'(x) = x^3(x-1)(x-2) \), \( \forall x \in \mathbb{R} \). Số điểm cực trị của hàm số đã cho là}{%
\begin{tasks}(4)
\task 1
\task 3
\task 5
...
\questionLinesManual{5}{10}{Cho hàm số \( y = f(x) \) có đạo hàm trên \( \mathbb{R} \). Biết hàm số \( y = f'(x) \) có đồ thị như hình vẽ. Phát biểu nào sau đây là đúng?
\begin{center}
\begin{tikzpicture}[>=latex, scale=0.8]
    % Hệ trục tọa độ
    \draw[->] (-3.5,0) -- (3.5,0) node[below] {$x$};
